from repos.calculator_repos import PowerRepository, FibonacciRepository, FactorialRepository
...
class CalculatorService:
    ...
    async def compute_power(self, data: PowRequest) -> CalculatorResponse:
        existing = await self._power_repository.get_result(data.base, data.exponent)
        if existing:
            return CalculatorResponse(
                message=f"Retrieved from database: pow({data.base}, {data.exponent})",
